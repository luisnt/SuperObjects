unit SuperObjects.PropertyEditor;

interface

uses System.Classes, DesignIntf,DesignEditors, superobject;
...
begin
  ...
  case ObjectGetType(node) of
    stObject:
      count := node.AsObject.count;
    stArray:
      count := node.AsArray.Length;
  else
    count := 0;
  end;
  if count > 0 then
    result := common + [paSubProperties, paVolatileSubProperties]
  else
    result := common;
end;
